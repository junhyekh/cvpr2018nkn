...
from model import EncoderDecoderGRU
...
def main(gpu, batch_size, alpha, beta, gamma, omega, margin, d_arch, d_rand,
  ...
  #     print(e)
    ...
    gru = EncoderDecoderGRU(batch_size, alpha, beta, gamma, omega, euler_ord,
                            ...
                            keep_prob, logs_dir, learning_rate, margin,
        ...
        for _, batchidx in mini_batches:
            ...
            if len(batchidx) == batch_size:
                ...
                dlf, dlr, gl, lc = gru.train(
                    torch.from_numpy(realSeq_batch).to(device), torch.from_numpy(realSkel_batch).to(device), 
                    torch.from_numpy(seqA_batch).to(device), torch.from_numpy(skelA_batch).to(device), 
                    torch.from_numpy(seqB_batch).to(device), torch.from_numpy(skelB_batch).to(device),
                    torch.from_numpy(aeReg_batch).to(device), torch.from_numpy(mask_batch).to(device), 
                    step)
